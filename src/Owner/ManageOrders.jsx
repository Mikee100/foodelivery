...
export default function ManageOrders() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await axios.get(`http://192.168.181.75:3000/api/orders?restaurantId=${restaurantId}`);
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.put(`http://192.168.181.75:3000/api/orders/${orderId}/status`, { status: newStatus }, {
